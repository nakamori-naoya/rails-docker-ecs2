...
プログラミング歴は2022年4月時点で1年半、職業エンジニアになってからは半年足らずですので新米も新米です。
...
## 軽く宣伝
もし仮に、私についてより詳しく知りたいということがございましたら、弊社playground株式会社で受けたインタビュー記事がありますので、読んでいただけますと幸いです。
[【社員インタビュー】プログラム未経験から独学でエンジニアに。きっかけから今後の目標まで聞いてみました！](https://www.wantedly.com/companies/playground/post_articles/375845)
また、テスト自動化を検討中の方、テスト自動化でお困りごとがある方は、私のTwitterアカウントへご連絡いただけますと幸いです。
...
- 自動テストツール、特にmablについて知りたいエンジニア・QA担当の皆様
...
3. それは、「デプロイ頻度」「サイクルタイム」「MTTR」です。
4. それら3つの指標を改善する24のケイパビリティがあるのでそれを実践していきましょう。**
...
![スクリーンショット 2022-05-09 0.46.36.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/499433/87a1da2a-1947-8f62-f425-007ba47cac87.png)
...
「推測するな、計測せよ」という格言にも従い、じゃあ計測しようではないか！となったわけです。
...
構造問題を解消していくことで、仕事仲間が幸せに働ける居場所づくりにつながればという思いもあります。
...
結果として、**フロントエンド・サーバーサイドともにユニットテストは書かれておらず、E2Eテストも書かれていない。**
- プロダクトの急成長に伴うエンジニアの人員増に比して**QAリソースは増えていない。**
- プロダクト数の増加ならびにプロダクト当たりの機能追加も加速度的に進んでおり、**手動E2Eテストのテスト項目も加速度的に増加している。**
- 別のプロダクトチームが大きな機能追加を行った場合の手動E2Eテストで**QAリソースが専有されてしまうことで、他のチームが小さな機能追加を行った場合の手動E2Eテストでさえも待ち時間が異常に長くなってしまう。**
- エンジニアがマインドマップでE2Eテストの項目を記載し、それをもとにQAチームがテストを行うので、**コミュニケーション齟齬が以前よりも頻繁に起きるようになっている。**
...
- 弊社の歴史としてCypressを使い自動テストを行ったことがあるが、エンジニアリソースを確保できずあえなく挫折している。
...
いくつかのツールを1週間ほど使用してみた結果、個人的には以下の理由からmablで決定となるくらいに圧倒的な差を感じてしまいました。
- mablのデスクトップアプリの**UI・UXが他サビースを圧倒していると感じた。**
- 基本的にはノーコードでテストを作成できる一方で、ほんの少しのプログラミングスキルと考え方を体得すれば、**非エンジニアでもメンテコストを限りなく小さくしたテストが書けると感じた。**
- 料金もリーズナブルで、**決済者の説得も比較的容易に感じた。**

最後に、これが決定的に重要なことなのですが、**mablは機能リリースの速度・量ともにその他のSaasサービスを圧倒しています。**
『LeanとDevOpsの科学』の調査結果から、エンジニア組織のパフォーマンスを左右するメトリクスとして、デプロイ頻度とサイクルタイムがあります。
**mablはこのデプロイ頻度とサイクルタイムにおいて他のサービスを圧倒していたのです。**
つまり、現段階でテスト自動化産業において、覇者になる確率が最も高いのはmablというわけです。

mablがさらにすごいところは細かな修正リリースではなくユーザーが心から欲しいと感じる機能を矢継ぎ早にリリースしている点にもあります。

# いざテスト自動化へ

## 超えなければいけないハードル
ツール選定としては、mabl一択となったのですが、超えるべきハードルがまだあります。
mablはSaaSサービスのため利用料金が発生するため、mablが費用対効果に十分見合うことを決済者に証明する必要があるのです。
...
## 辛いQAフェーズ
従来までは、機能追加の度にマインドマップにエンジニアがテストケースを追加していました。
それをプロダクトチームと独立して存在するQAチームがテストを行うという形でした。
前述の通り、これはコミュニケーションコストを増大させるばかりか、プロダクトの機能が正しく動くことを保証するものですらありません。
私にとってマインドマップを書く行為は意味が感じられず本当に苦痛でした。
...
私にとってmablが非常に魅力的に映ったのは、**プログラミング的な発想さえあれば、ノーコードでも非常にメンテナンスコストの低いテストが作れると思えたからです。**
